...
public class NIOBankData implements BankDataInterface {
	...
	public BankAccount read(int position) throws IOException {
		...
		file.read(buffer);
		buffer.rewind();
		byte[] myString = new byte[CHAR_SIZE*STRING_LENGHT];
		buffer.get(myString);
		return new BankAccount(buffer.getInt(), buffer.getDouble(),buffer.getFloat(), new String(myString));
	}
	...
	public void write(int position, BankAccount account) throws IOException {
		...
		byte[] myBytes = new byte[CHAR_SIZE*STRING_LENGHT];
		byte[] myString = account.getName().getBytes(CHARSET);
		for (int i = 0; i<Math.min(myString.length, myBytes.length); i++) {
			myBytes[i]=myString[i];
		}
		buffer.put(myBytes);
		buffer.rewind();
